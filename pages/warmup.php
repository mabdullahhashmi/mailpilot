...
// Fetch active warmup accounts
$accounts = dbFetchAll("SELECT id, label, from_email, warmup_status, warmup_current_day, warmup_target_daily, is_seed_account, sent_today FROM smtp_accounts WHERE warmup_status = 'active' OR is_seed_account = 1 ORDER BY is_seed_account DESC, warmup_current_day DESC");
...
<div class="page-header">
    <div>
        ...
        <div class="subtitle">Automated domain reputation building using the 14-day Smart Ramp-up Algorithm</div>
    </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!$acc['is_seed_account']): ?>
                                    <div style="font-size: 13px; margin-bottom: 4px;">Day <?= $acc['warmup_current_day'] ?> of 14</div>
                                    <div style="width: 100%; height: 6px; background: var(--border-color); border-radius: 3px; overflow: hidden;">
                                        <div style="height: 100%; background: var(--color-primary); width: <?= min(100, ($acc['warmup_current_day'] / 14) * 100) ?>%;"></div>
                                    </div>
                                <?php else: ?>
...
<div class="card">
    ...
    <div class="card-body" style="line-height: 1.6; color: var(--text-secondary);">
        ...
        <p>3. <strong>The Algorithm:</strong> MailPilot randomly generates free business inquiries (using the built-in Spintax engine). It sends them across your network, ensuring domains never email themselves. Over 14 days, the volume slowly and safely increases.</p>
